...
public class OpenCourseRequest extends JSONRequest {

	private Handler handler;
	private String wareid;
	private Context context;
	private UserBean user=UserBean.getInstance();
	public OpenCourseRequest(Handler handler,Context context) {
		super(handler);
		this.handler = handler;
		this.context=context;
	}

	public void setCourseId(String wareid) {
		this.wareid = wareid;
	}
	...
	protected void onHttpSuccess(String str) {
		...
		try {
			...
			user.setLogid(logid);
			if ("1".equals(code)) {
				handler.sendEmptyMessage(HandleMessageState.OPEN_COURSE_SUCCESS);
			} else if ("-1".equals(code)) {
				handler.sendEmptyMessage(HandleMessageState.OPEN_COURSE_FAILURE);
			}
