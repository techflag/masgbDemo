...
import com.broov.player.masgb.entity.UpLoadTimeInfo;
import com.broov.player.masgb.entity.UpLoadTimeJsonUtil;
...
@SuppressLint("NewApi")
public class VideoPlayerOffLineActivity extends Activity implements
		...
		public void run() {
			...
			if (!isFinishing()) {
				AlertDialog.Builder builder = new AlertDialog.Builder(
						VideoPlayerOffLineActivity.this);
				...
				builder.setNegativeButton("取消",
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog,
									int which) {
								String wareId = makeWareId(tmpFileName);
								makeDownloadData(id, startTime, endTime, wareId);
								finish();
							}
						});
				...
				builder.setPositiveButton("确定",
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog,
								...
								bundle.putString("from", "offlinevideo");
	...
	private void makeDownloadData(String studentId, String startTime,
			String endTime, String wareId) {
		ArrayList<UpLoadTimeInfo> list = new ArrayList<UpLoadTimeInfo>();

		sharedPreferences = getSharedPreferences("upTime", Context.MODE_PRIVATE);
		// getString()第二个参数为缺省值，如果preference中不存在该key，将返回缺省值
		String jsonArrayStart = sharedPreferences.getString("uptimejson", "");
		if (!jsonArrayStart.equals(null)) {
			list = UpLoadTimeJsonUtil.changeJsonToArray(jsonArrayStart);
		}

		UpLoadTimeInfo dl1 = new UpLoadTimeInfo(studentId, startTime, endTime,
				wareId);
		list.add(dl1);
		String jsonArrayEnd = UpLoadTimeJsonUtil.changeArrayDateToJson(list);

		sharedPreferences = getSharedPreferences("upTime", Context.MODE_PRIVATE);
		Editor editor = sharedPreferences.edit();// 获取编辑器
		editor.putString("uptimejson", jsonArrayEnd);
		editor.commit();// 提交修改
	}
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		tmpFileName = bundle.getString("videofilename");
		...
		try {
			...
			if (mLibVLC != null) {
				String pathUri = LibVLC.getInstance().nativeToURI(tmpFileName);
				//mLibVLC.readMedia(pathUri, false);
				Toast.makeText(this, ""+tmpFileName, Toast.LENGTH_LONG).show();
				//mLibVLC.playMRL(tmpFileName);
				
				loadMedia(tmpFileName);
...
//        itemPosition = getIntent().getExtras().getInt("itemPosition", -1);
      ...
      //mLocation = "http://f.youku.com/player/getFlvPath/sid/00_00/st/flv/fileid/03000201005372BFB56E3D08EB51973DAE3BB8-18AE-8DC7-435B-B8EA5FEA673F?K=57e5fabce070302b282989ee";
        ...
        mLibVLC.playMRL("file://"+mLocation);
        
    }
	
	private void encounteredError() {
        /* Encountered Error, exit player with a message */
        AlertDialog dialog = new AlertDialog.Builder(VideoPlayerOffLineActivity.this)
	...
	private static class VideoPlayerHandler extends
			WeakHandler<VideoPlayerOffLineActivity> {
		public VideoPlayerHandler(VideoPlayerOffLineActivity owner) {
			super(owner);
		}

		@Override
		public void handleMessage(Message msg) {
			VideoPlayerOffLineActivity activity = getOwner();
	...
	private static class VideoPlayerEventHandler extends
			WeakHandler<VideoPlayerOffLineActivity> {
		public VideoPlayerEventHandler(VideoPlayerOffLineActivity owner) {
			super(owner);
		}
