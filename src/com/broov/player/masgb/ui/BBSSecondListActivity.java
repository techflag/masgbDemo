...
public class BBSSecondListActivity extends BaseActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mContext = BBSSecondListActivity.this;
	...
	public void initData() {
		...
		seekBBSRequest = new SeekBBSSecondRequest(handler, boardId, index + 1
				+ "", 4 + "");
		seekBBSRequest.sendRequest();
		setPage(index + 1);
		playProgressDialog(mContext);
	...
	public void onClick(View v) {
		...
		if (v == upBBSBtn) {
			...
			llListView.setVisibility(View.VISIBLE);
			ALLBBS = "allbbs";
			seekBBSRequest = new SeekBBSSecondRequest(handler, boardId, index
					+ 1 + "", 4 + "");
			seekBBSRequest.sendRequest();
			setPage(index + 1);
			playProgressDialog(mContext);
			...
			SendNewBBSRequest sendNewBBSRequest = new SendNewBBSRequest(
					...
					edtSendContentInSecond.getText().toString(), edtSendTitle
							.getText().toString(), getLocalIpAddress());
